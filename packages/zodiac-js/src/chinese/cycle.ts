/**
 * Chinese Zodiac 60-year sexagenary cycle module.
 *
 * Provides reference data and lookup tables for the Chinese Zodiac cycle,
 * including Chinese New Year dates for accurate boundary calculations.
 */

/** Earthly Branch order (12 animals) */
export const EARTHLY_BRANCHES = [
  'rat', 'ox', 'tiger', 'rabbit', 'dragon', 'snake',
  'horse', 'goat', 'monkey', 'rooster', 'dog', 'pig',
] as const;

/** Heavenly Stem order (5 elements × yin/yang = 10 stems) */
export const HEAVENLY_STEMS = [
  'wood-yang', 'wood-yin', 'fire-yang', 'fire-yin', 'earth-yang',
  'earth-yin', 'metal-yang', 'metal-yin', 'water-yang', 'water-yin',
] as const;

/** Reference year for cycle calculations. */
export const REFERENCE_YEAR = 1984; // Wood Rat Yang, cycle position 1

/**
 * Chinese New Year dates (1900–2100).
 * Key = year number, value = "YYYY-MM-DD" ISO date string.
 */
export const CHINESE_NEW_YEAR_DATES: Record<number, string> = {
  ...
  2052: '2052-02-01', 2053: '2053-02-19', 2054: '2054-02-08', 2055: '2055-01-28',
  2056: '2056-02-15', 2057: '2057-02-04', 2058: '2058-01-24', 2059: '2059-02-12',
  2060: '2060-02-02', 2061: '2061-01-21', 2062: '2062-02-09', 2063: '2063-01-29',
  2064: '2064-02-17', 2065: '2065-02-05', 2066: '2066-01-26', 2067: '2067-02-14',
  2068: '2068-02-03', 2069: '2069-01-23', 2070: '2070-02-11', 2071: '2071-01-31',
  2072: '2072-02-19', 2073: '2073-02-07', 2074: '2074-01-27', 2075: '2075-02-15',
  2076: '2076-02-05', 2077: '2077-01-24', 2078: '2078-02-12', 2079: '2079-02-02',
  2080: '2080-01-22', 2081: '2081-02-09', 2082: '2082-01-29', 2083: '2083-02-17',
  2084: '2084-02-06', 2085: '2085-01-26', 2086: '2086-02-14', 2087: '2087-02-03',
  2088: '2088-01-24', 2089: '2089-02-10', 2090: '2090-01-30', 2091: '2091-02-18',
  2092: '2092-02-07', 2093: '2093-01-27', 2094: '2094-02-15', 2095: '2095-02-05',
  2096: '2096-01-25', 2097: '2097-02-12', 2098: '2098-02-01', 2099: '2099-01-21',
  2100: '2100-02-09',
};
...
/**
 * Returns the Chinese New Year date for a given Gregorian year.
 * @returns ISO date string (YYYY-MM-DD) or null if year is out of range.
 */
export function getChineseNewYearDate(year: number): string | null {
  return CHINESE_NEW_YEAR_DATES[year] ?? null;
}

/**
 * Determines the Chinese Zodiac year for a given Gregorian date.
 *
 * If the date falls before Chinese New Year in that Gregorian year,
 * the Chinese year is the previous year's Chinese zodiac year.
 *
 * @param year Gregorian year
 * @param month Gregorian month (1-12)
 * @param day Gregorian day (1-31)
 * @returns The Chinese zodiac year (number) for the given date
 */
export function getChineseYear(year: number, month: number, day: number): number {
  const cnyDate = CHINESE_NEW_YEAR_DATES[year];
  if (!cnyDate) {
    // If we don't have CNY date, use mathematical approximation
    return year;
  }

  const [, cnyMonth, cnyDay] = cnyDate.split('-').map(Number);
  const inputDayOfYear = month * 100 + day;
  const cnyDayOfYear = cnyMonth * 100 + cnyDay;

  // If the date is before Chinese New Year, it belongs to the previous Chinese year
  return inputDayOfYear < cnyDayOfYear ? year - 1 : year;
}

/**
 * Gets the animal ID for a given Chinese year.
 */
export function getAnimalForYear(chineseYear: number): string {
  // The reference year 1984 is a Rat year (index 0)
  const offset = ((chineseYear - REFERENCE_YEAR) % 12 + 12) % 12;
  return EARTHLY_BRANCHES[offset];
}

/**
 * Gets the element ID and yin/yang for a given Chinese year.
 * The Heavenly Stem cycles every 10 years:
 *   0,1 = wood (yang,yin), 2,3 = fire, 4,5 = earth, 6,7 = metal, 8,9 = water
 */
export function getStemForYear(chineseYear: number): { element: string; yinYang: 'yin' | 'yang' } {
  const offset = ((chineseYear - REFERENCE_YEAR) % 10 + 10) % 10;
  const stem = HEAVENLY_STEMS[offset];
  const [element, yinYang] = stem.split('-') as [string, 'yin' | 'yang'];
  return { element, yinYang };
}

/**
 * Gets the position within the 60-year sexagenary cycle (1-60).
 */
export function getCyclePosition(chineseYear: number): number {
  const offset = ((chineseYear - REFERENCE_YEAR) % 60 + 60) % 60;
